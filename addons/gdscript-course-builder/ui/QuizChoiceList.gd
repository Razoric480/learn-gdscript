...
const QuizChoiceItem := preload("res://addons/gdscript-course-builder/ui/QuizChoiceItem.gd")
...
func _add_answer(answer := "") -> void:
	var instance: QuizChoiceItem = QuizChoiceScene.instantiate()
	add_child(instance)
	...
	instance.index_changed.connect(_update_answer_labels)
	instance.choice_changed.connect(_update_quiz_answers)
	instance.removed.connect(_update_quiz_answers)
