...
func _ready() -> void:
	_add_button.connect("pressed", Callable(self, "_add_answer"))
...
func setup(quiz: QuizChoice) -> void:
	...
	if not is_inside_tree():
		await self.ready
	_quiz.connect("choice_type_changed", Callable(self, "_update_children_checkboxes"))
...
func _add_answer(answer := "") -> void:
	var instance = QuizChoiceScene.instantiate()
	add_child(instance)
	...
	instance.connect("index_changed", Callable(self, "_update_answer_labels"))
	instance.connect("choice_changed", Callable(self, "_update_quiz_answers"))
	instance.connect("removed", Callable(self, "_update_quiz_answers"))
